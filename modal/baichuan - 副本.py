...
def get(question: Dict):
    ...
    model = baichuan()
    call = model.generate.call(question["question"])
    return RedirectResponse("https://xqy2006--chatxu-1-get.modal.run/result/"+call.object_id)
...
async def poll_results(call_id: str):
    ...
    try:
        return StreamingResponse(
            function_call.get(timeout=149),
            media_type="text/event-stream",
    )
    except TimeoutError:
        return RedirectResponse("https://xqy2006--chatxu-1-get.modal.run/result/"+call_id)