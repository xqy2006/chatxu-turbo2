...
IMAGE_MODEL_DIR = "/model1"

from typing import Dict
def download_model():
    from huggingface_hub import snapshot_download
    import os
    model_name = "huangyuyang/chatglm2-6b-int4.flm"
    snapshot_download(model_name, local_dir=IMAGE_MODEL_DIR)
    snapshot_download("xuqinyang/fastllm", local_dir="./")
    os.system("git clone https://huggingface.co/xuqinyang/fastllm /model1")
    os.system("python /model1/tools/setup.py install")


image = (
    Image.debian_slim(python_version="3.10")
    .apt_install("git").apt_install("gcc").apt_install("cmake").apt_install("g++")
    .pip_install(
        "einops==0.6.1",
        "hf-transfer~=0.1",
...
stub = Stub(name="chatxu_new", image=image)

@stub.cls(cpu=4, timeout=300, container_idle_timeout=120)
class baichuan:
    def __enter__(self):
        import llm
        self.tokenizer = AutoTokenizer.from_pretrained(IMAGE_MODEL_DIR, trust_remote_code=True)
        self.model = llm.model("/model/chatglm2-6b-int4.flm")
    @method()
    def generate(self, req: str):
        ...
        response = self.model.response(prompt,history = history)
